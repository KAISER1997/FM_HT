...
def _extreme_inverse_and_lad(x, tail_param):
    ...
    nan11=torch.isnan(g).any()
    if nan11:
        print("g has nan in exinv")
    log_g = -torch.log(inner) / tail_param
    erfcinv_val = _stable_erfcinv(g, log_g)
    nan12=torch.isnan(erfcinv_val).any()
    if nan12:
        print("ercinv_val has nan")
...
def dTTF_dz(z, pos_tail, neg_tail,shift,scale): #aditya wrote this, checked 1 time
    sqrt_2 = SQRT_2
...
def _tail_affine_inverse(x, pos_tail, neg_tail, shift, scale):
    ...
    nan8=torch.isnan(x).any()
    nan9=torch.isnan(tail_param).any()
    nan10=torch.isnan(z).any()
    if nan8:
        print("x in tailinv has nan")
    if nan9:
        print("tail_param has nan")
    if nan10:
        print("z in tail_aff_inv has nan")
...
class TailAffineMarginalTransform(nn.Module):
    ...
    def forward(self, z,mode='default', context=None):
    ...
    def inverse(self, x,mode='default', context=None):
        if mode=='default':
            """heavy -> light"""
...
def grad_R(z,  lambda_p, lambda_n,mu, sigma): #checked one time
    s = torch.sign(z)
    abs_z = torch.abs(z)
    sqrt_2 = torch.sqrt(torch.tensor(2.0, device=z.device))
    erfc_term = torch.erfc(abs_z / sqrt_2)+0.0001 #Aditya has added the extra 0.000..1 to handle zero power negative in the next line
    log_erfc = torch.log(erfc_term)
    
    # Common terms
    lambda_s = torch.where(s == 1, lambda_p, lambda_n)+0.00001 #Aditya has added the extra 0.000..1 to handle denominator in the next line
    ...
    has_nan1 = torch.isnan(dR_dmu).any()
    has_nan2 = torch.isnan(dR_dsigma).any()
    has_nan3 = torch.isnan(dR_dlambda_p).any()
    has_nan4 = torch.isnan(dR_dlambda_n).any()    

    if has_nan1:
        print("nan1")
    if has_nan2:
        print("nan2")
        print("z",torch.min(abs_z),torch.max(abs_z))
    if has_nan3:
        print("nan3")
    if has_nan4:
        print("nan4")
    ...
    return dR_dlambda_p, dR_dlambda_n,dR_dmu, dR_dsigma #SUPER IMPORTANT CHANGED ON 7MAY
