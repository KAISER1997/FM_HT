# =============================================================
# = Credits : https://github.com/ivanvoitalov/tail-estimation =
# =============================================================
import sys
import time
import argparse
...
def make_plots(ordered_data, output_file_path, number_of_bins,
    ...
    tail_estimators = {}
    if xi_h_star > 0:
        tail_estimators["hill"] = 1 + 1./xi_h_star
    else:
        tail_estimators["hill"] = 0
    if xi_m_star > 0:
        tail_estimators["moments"] = 1 + 1./xi_m_star
    else:
        tail_estimators["moments"] = 0
    if xi_k_star > 0:
        tail_estimators["kernel"] = 1 + 1./xi_k_star
    else:
        tail_estimators["kernel"] = 0
    return(tail_estimators)
...
def main():
    ...
    parser.add_argument("--path_estimator",
                        help="If specified then save the tail index at specified location.",
                        type=str, default="")
    ...
    hsteps = args.hsteps
    path_estimator = args.path_estimator
    if args.noise == 1:
        noise_flag = True
    else:
        noise_flag = False
    ...
    print("========================")
    tail_estimators = make_plots(ordered_data, args.output_file_path, number_of_bins,
               r_smooth, alpha, hsteps, bootstrap_flag, t_bootstrap,
    ...
    if tail_estimators["moments"] == tail_estimators["kernel"] == 0:
        ...
        if tail_estimators["hill"]!=0:
            final_estimator = tail_estimators["hill"]
