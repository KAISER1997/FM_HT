...
class MLP_W_ttflayers(nn.Module):
    def __init__(self,base_model, input_dim: int = 2, time_dim: int = 1, hidden_dim: int = 128,transform='basic'):
        super().__init__()
        ...
        self.TTF=TTF_layer(input_dim,transform)
    ...
    def forward(self, x: Tensor, t: Tensor) -> Tensor:
        sz = x.size()
        output=self.TTF(self.base_model(x,t))
        # print(output.shape,"hello")
        # x = x.reshape(-1, self.input_dim)
        # t = t.reshape(-1, self.time_dim).float()
